...
1. Write a function named `makeTea` that takes one parameter, `typeOfTea` , and returns 
    a string like `"Making green tea"` when called with `"green tea"`.
...
3. Write an arrow function named `calculateTotal` that takes two parameters: `price` and 
    `quantity`. The function should return the total cost by multiplying
...
const totalCost = calculateTotal(10,30);
...
function makeTea(typeTea){
        return `This is maketea: ${typeTea}`
}
